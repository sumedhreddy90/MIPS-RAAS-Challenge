...
class ArucoWrapper:
    ...
    def __init__(self, marker_length, camera_matrix, dist_coeffs, aruco_dictionary_name="DICT_5X5_50"):
    ...
    def find_poses_from_corners(self, aruco_ids, aruco_corners, image_color=None, draw_image=False):
        ...
        if aruco_ids is not None and len(aruco_ids) > 0:
            ...
            rvecs, tvecs, _objPoints = cv2.aruco.estimatePoseSingleMarkers(aruco_corners,
                                                                       self.marker_length,
                                                                       self.camera_matrix,
                                                                       self.dist_coeffs)
            ...
            for i in range(len(aruco_ids)):
                ...
                if draw_image is True and image_color is not None:
                    image_color = cv2.aruco.drawAxis(image_color,
                                                self.camera_matrix,
                                                self.dist_coeffs,
                                                rvecs[i], tvecs[i],
